...
| **Monitor de bandeja** | `windows-usbip-broker-cpp/build/` | O MSI já inclui o launcher `UsbipTrayLauncher.exe` e o monitor de bandeja |
...
Este repositório foi reduzido para foco em instalação. O MSI já está pronto em:

```text
windows-usbip-broker-cpp\build\UsbipSuite-2.0.0-x64.msi
```

E o instalador Linux está em:

```text
linux-usbip-manager\install.sh
```
